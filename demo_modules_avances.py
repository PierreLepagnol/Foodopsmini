...
def demo_marketing_system():
    ...
    for customer_id, rating, comment, platform in reviews_data:
        marketing.add_customer_review(customer_id, "resto_001", Decimal(str(rating)), comment, platform)
...
def demo_finance_system():
    ...
    recipes_data = [
        ("burger_001", "Burger Classic", Decimal("12.50"), Decimal("4.20"), Decimal("2.80"), Decimal("1.50")),
        ("pizza_001", "Pizza Margherita", Decimal("11.00"), Decimal("3.80"), Decimal("2.20"), Decimal("1.40")),
        ("salad_001", "Salade César", Decimal("9.80"), Decimal("3.10"), Decimal("1.90"), Decimal("1.20")),
        ("pasta_001", "Pâtes Carbonara", Decimal("10.50"), Decimal("2.90"), Decimal("2.10"), Decimal("1.30"))
    ]
    
    for recipe_id, name, price, ingredient_cost, labor_cost, overhead_cost in recipes_data:
        finance.update_recipe_profitability(recipe_id, name, price, ingredient_cost, labor_cost, overhead_cost)
    ...
    for recipe in profitability_report:
        print(f"   {recipe['recipe_name']:<20} | {recipe['selling_price']:>6.2f}€ | {recipe['total_cost']:>6.2f}€ | {recipe['unit_margin']:>6.2f}€ | {recipe['margin_rate']:>6.1%} | {recipe['quantity_sold']:>8} | {recipe['total_profit']:>8.2f}€")
    ...
    for forecast in cash_flow['forecast']:
        print(f"   J+{forecast['day']:<4} | {forecast['date'].strftime('%d/%m/%Y'):<12} | {forecast['cash_position']:>10.2f}€ | {forecast['daily_flow']:>+8.2f}€")
