...
from Foodopsmini import (
    Restaurant, allocate_demand, compute_pnl, 
    get_restaurant_quality_score, BASE_DEMAND
)
...
def test_quality_impact():
    ...
    for turn in range(1, 6):
        ...
        for r in restaurants:
            res = results[r.name]
            print(f"  {r.name}:")
            print(f"    Clients: {res['served']}, CA: {res['revenue']:.0f}€, Profit: {res['profit']:.0f}€")
            print(f"    Satisfaction: {res['satisfaction']:.1f}/5, Réputation: {res['reputation']:.1f}/10")
    ...
    for r in restaurants:
        ...
        market_share = results[r.name]['served'] / total_served * 100 if total_served > 0 else 0
...
def test_strategic_scenarios():
    ...
    for r in restaurants_scenario1:
        ...
        print(f"    Qualité: {quality_score:.1f}/5, Satisfaction: {pnl['customer_satisfaction']:.1f}/5")
