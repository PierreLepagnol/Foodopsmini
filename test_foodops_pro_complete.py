...
def create_test_scenario():
    ...
    segments = [
        ...
        MarketSegment(
            ...
            type_preferences={"gastronomique": Decimal("1.8"), "classic": Decimal("1.2")}
        )
    ]
...
def test_quality_system():
    ...
    for resto in restaurants:
        ...
        if resto.ingredient_choices:
            ingredients_desc = ", ".join([f"{k}:{v}⭐" for k, v in resto.ingredient_choices.items()])
    ...
    for turn in range(1, 4):
        ...
        for resto in restaurants:
            if resto.id in results:
                ...
                market_share = (result.served_customers / total_served * 100) if total_served > 0 else 0
                
                print(f"\n  {resto.name}:")
                print(f"    Clients: {result.served_customers}/{result.capacity} ({result.utilization_rate:.0%})")
                print(f"    CA: {result.revenue:.0f}€ (ticket: {result.average_ticket:.2f}€)")
