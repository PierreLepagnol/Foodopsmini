...
def sample_social_charges_config():
    ...
    return {
        ...
        "cdd": {"employee_rate": Decimal("0.22"), "employer_rate": Decimal("0.44")},
        "extra": {"employee_rate": Decimal("0.22"), "employer_rate": Decimal("0.45")},
        "apprenti": {
            "employee_rate": Decimal("0.05"),
            "employer_rate": Decimal("0.11"),
        },
        "stage": {"employee_rate": Decimal("0.00"), "employer_rate": Decimal("0.00")},
    }
...
class TestPayrollCalculator:
    ...
    def test_basic_payroll_calculation_cdi(
        self, sample_social_charges_config, sample_employees
    ):
        ...
        assert abs(
            result.social_charges_employee - expected_employee_charges
        ) < Decimal("0.01")
        assert abs(
            result.social_charges_employer - expected_employer_charges
        ) < Decimal("0.01")
    ...
    def test_payroll_calculation_apprenti(
        self, sample_social_charges_config, sample_employees
    ):
    ...
    def test_payroll_calculation_part_time(
        self, sample_social_charges_config, sample_employees
    ):
        ...
        expected_effective_salary = (
            employee.salary_gross_monthly * employee.part_time_ratio
        )
        assert result.gross_salary == expected_effective_salary

        # Les charges sont calculées sur le salaire effectif
        expected_employer_charges = expected_effective_salary * Decimal(
            "0.45"
        )  # Taux extra
        assert abs(
            result.social_charges_employer - expected_employer_charges
    ...
    def test_overtime_calculation_eligible(
        self, sample_social_charges_config, sample_employees
    ):
    ...
    def test_overtime_calculation_not_eligible(
        self, sample_social_charges_config, sample_employees
    ):
    ...
    def test_sunday_premium_not_authorized(
        self, sample_social_charges_config, sample_employees
    ):
    ...
    def test_complex_payroll_calculation(
        self, sample_social_charges_config, sample_employees
    ):
        ...
        expected_gross = (
            employee.salary_gross_monthly + result.overtime_pay + result.sunday_premium
        )
    ...
    def test_team_payroll_calculation(
        self, sample_social_charges_config, sample_employees
    ):
        ...
        results = calculator.calculate_team_payroll(
            sample_employees, hours_per_employee=hours_per_employee, period="2024-01"
        )
    ...
    def test_payroll_summary(self, sample_social_charges_config, sample_employees):
        ...
        expected_rate = (
            (manual_total_charges / manual_total_gross * 100)
            if manual_total_gross > 0
            else Decimal("0")
        )
    ...
    def test_overtime_rates_application(
        self, sample_social_charges_config, sample_employees
    ):
        ...
        expected_overtime_pay = hours_25 * hourly_rate * (
            calculator.overtime_rate_25 - 1
