    ...
    def test_data_loading_integration(self):
        ...
        for recipe in recipes.values():
            for item in recipe.items:
                assert item.ingredient_id in ingredients, f"Ingrédient {item.ingredient_id} manquant pour la recette {recipe.id}"
    ...
    def test_recipe_costing_with_real_data(self):
        ...
        for recipe_id, recipe in data['recipes'].items():
            try:
                ...
                manual_total = sum(ic.total_cost_ht for ic in breakdown.ingredient_costs)
