...
class TestAccount:
    ...
    def test_account_creation(self):
        ...
        account = Account(
            number="701", name="Ventes de marchandises", type=AccountType.REVENUE
        )
...
class TestAccountingEntry:
    ...
    def test_entry_creation_same_accounts(self):
        ...
        with pytest.raises(
            ValueError, match="Les comptes débit et crédit doivent être différents"
        ):
            AccountingEntry(
                date=date.today(),
    ...
    def test_entry_creation_empty_accounts(self):
        ...
        with pytest.raises(
            ValueError, match="Les comptes débit et crédit sont obligatoires"
        ):
            AccountingEntry(
                date=date.today(),
...
class TestLedger:
    ...
    def test_record_sale_basic(self):
        ...
        assert abs(ledger.get_balance("706") - expected_ht) < Decimal(
            "0.01"
        )  # Prestations
        assert abs(ledger.get_balance("44571") - expected_vat) < Decimal(
            "0.01"
        )  # TVA collectée
    ...
    def test_record_purchase_basic(self):
        ...
        assert abs(ledger.get_balance("44566") - expected_vat) < Decimal(
            "0.01"
        )  # TVA déductible
        assert abs(ledger.get_balance("401") - expected_ttc) < Decimal(
            "0.01"
        )  # Fournisseurs
    ...
    def test_record_payroll(self):
        ...
        ledger.record_payroll(
            gross_salary, social_charges, payroll_date, "Paie janvier"
        )
    ...
    def test_get_trial_balance(self):
        ...
        ledger.record_purchase(
            Decimal("50.00"), Decimal("0.055"), date.today(), "Achat 1"
        )
    ...
    def test_get_profit_loss(self):
        ...
        ledger.record_purchase(
            Decimal("400.00"), Decimal("0.055"), date.today(), "Achats"
        )
        ledger.record_payroll(
            Decimal("2000.00"), Decimal("840.00"), date.today(), "Paie"
        )
        ...
        expected_expenses = (
            Decimal("400.00") + Decimal("2000.00") + Decimal("840.00")
        )  # Achats + salaires + charges
