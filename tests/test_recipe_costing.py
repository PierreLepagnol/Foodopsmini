...
from src.foodops_pro.core.costing import RecipeCostCalculator, CostBreakdown, IngredientCost
...
class TestRecipeCostCalculator:
    ...
    def test_recipe_cost_with_stock(self, sample_ingredients, sample_recipe, sample_stock_lots):
        ...
        beef_cost = next(
            cost for cost in breakdown.ingredient_costs 
            if cost.ingredient_id == "beef"
        )
    ...
    def test_labor_cost_calculation(self, sample_ingredients, sample_recipe):
        ...
        assert abs(breakdown.preparation_time_cost - expected_labor_cost) < Decimal("0.01")
