...
def simulate_competition_example():
    ...
    segments = [
        {
            ...
            "clients": 147,
            "budget": 11.0,
            "affinite": {"fast": 1.2, "classic": 0.7},
        },
        {
            "name": "Familles",
            "clients": 168,
            "budget": 17.0,
            "affinite": {"fast": 0.9, "classic": 1.0},
        },
        {
            "name": "Foodies",
            "clients": 105,
            "budget": 25.0,
            "affinite": {"fast": 0.6, "classic": 1.3},
        },
    ]
    ...
    for segment in segments:
        ...
        for r in restaurants:
            ...
            if r["prix"] <= segment["budget"]:
                facteur_prix = (
                    1.0 + (segment["budget"] - r["prix"]) / segment["budget"] * 0.3
                )
            ...
            print(
                f"  {r['name']}: affinité {affinite:.1f} × prix {facteur_prix:.2f} = {score:.2f}"
            )
        ...
        if total_score > 0:
            for r in restaurants:
                ...
                print(
                    f"    → {r['name']}: {allocation:.0f} clients ({allocation / segment['clients']:.1%})"
                )
    ...
    for r in restaurants:
        ...
        part_marche = (
            servi
            / sum(
                min(int(total_allocation[resto["name"]]), resto["capacite"])
        ...
        print(
            f"  {r['name']}: {servi}/{demande} clients, CA {ca:.0f}€, part {part_marche:.1f}%"
        )
        if perdu > 0:
