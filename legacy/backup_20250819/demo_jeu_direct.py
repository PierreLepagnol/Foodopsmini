...
def demo_tour_complet():
    ...
    for segment, data in segments.items():
        ...
        # Clients du segment
        segment_clients = int(
            data["clients_potentiels"] * 0.3 * attractiveness * quality_bonus
        )
        total_clients += segment_clients

        print(
            f"   {segment}: {segment_clients} clients (prix {price_ratio:.1f}x budget)"
        )
...
def demo_strategies():
    ...
    print(
        f"{'Stratégie':<12} | {'Prix':<6} | {'Qualité':<7} | {'Clients':<8} | {'Profit':<8} | {'Marge':<6}"
    )
    print("-" * 70)
    ...
    for strategy in strategies:
        ...
        ingredient_costs = {1: 2.94, 2: 4.20, 3: 5.25, 4: 6.30, 5: 8.40}
        costs = (
            ingredient_costs[strategy["quality"]] * clients
            + {1: 2200, 2: 2800, 3: 3600}[strategy["staff"]]
            + 1200
        )
        ...
        print(
            f"{strategy['name']:<12} | {strategy['price']:>5.2f}€ | {strategy['quality']:>7}/5 | {clients:>8} | {profit:>+7.0f}€ | {margin:>5.1f}%"
        )
