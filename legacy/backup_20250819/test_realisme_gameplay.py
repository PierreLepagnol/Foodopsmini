...
def simulate_restaurant_purchase():
    ...
    for commerce in commerces:
        ...
        print(
            f"   Prix: {commerce.price:,}€ + {commerce.renovation_cost:,}€ rénovation = {total_cost:,}€"
        )
        print(f"   Capacité: {commerce.capacity} couverts")
        print(f"   Loyer: {commerce.rent_monthly:,}€/mois")
        print(
            f"   Trafic: {commerce.traffic_level}, Concurrence: {commerce.competition_level}"
        )
    ...
    for recipe_id, recipe_name in menu_recipes:
        if recipe_id in data["recipes"]:
            ...
            market_price = Decimal(
                str(market_prices.get(recipe_id, float(suggested_price)))
            )
            ...
            print(
                f"   {recipe_name}: {market_price}€ (coût {cost_breakdown.total_cost_with_labor:.2f}€, food cost {food_cost_pct:.1f}%)"
            )
    ...
    for turn in range(1, num_turns + 1):
        ...
        for recipe_id in restaurant.active_recipes:
            if recipe_id in data["recipes"]:
                ...
                recipe_sales = player_result.served_customers // len(
                    restaurant.active_recipes
                )
                variable_costs += (
                    float(cost_breakdown.total_cost_with_labor) * recipe_sales
                )

        # Coûts fixes journaliers
        monthly_fixed = (
            float(restaurant.rent_monthly) + 8000
        )  # Loyer + salaires estimés
        ...
        for comp in competitors:
            ...
            print(
                f"  {comp.name}: {comp_result.served_customers} clients, {comp_result.revenue:.0f}€"
            )
