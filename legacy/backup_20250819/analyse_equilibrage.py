...
def analyze_recipe_costs():
    ...
    for recipe_id in key_recipes:
        if recipe_id in data["recipes"]:
            ...
            suggested_price = cost_breakdown.total_cost_with_labor / Decimal(
                str(food_cost_target)
            )
            ...
            for price in [10, 15, 20, 25]:
                ...
                print(
                    f"   À {price}€: Food cost {food_cost_pct:.1f}%, Marge {margin_analysis['margin_percentage'] * 100:.1f}%"
                )
...
def analyze_profitability():
    ...
    for daily_customers in [30, 50, 80, 120]:
        ...
        burger_cost = calculator.calculate_recipe_cost(
            data["recipes"]["burger_classic"]
        )
        pasta_cost = calculator.calculate_recipe_cost(
            data["recipes"]["pasta_bolognese"]
        )
