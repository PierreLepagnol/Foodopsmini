...
class MarketSimulator:
    ...
    def simulate_demand(self, restaurant):
        ...
        for segment_name, segment in self.segments.items():
            ...
            segment_clients = int(
                base_demand * attractiveness * random.uniform(0.8, 1.2)
            )
...
class GameEngine:
    ...
    def start_game(self):
        ...
        name = input(
            "Nom de votre restaurant (ou Entrée pour 'Mon Restaurant'): "
        ).strip()
        if not name:
            name = "Mon Restaurant"
    ...
    def make_decisions(self):
        ...
        try:
            ...
            print(
                "   Segments clientèle: Étudiants (~11€), Familles (~17€), Foodies (~25€)"
            )
            new_price = input(
                f"Nouveau prix (ou Entrée pour garder {self.restaurant.price}€): "
            ).strip()
            ...
            new_quality = input(
                f"Niveau qualité 1-5 (ou Entrée pour garder {self.restaurant.quality_level}): "
            ).strip()
            ...
            new_staff = input(
                f"Niveau personnel 1-3 (ou Entrée pour garder {self.restaurant.staff_level}): "
            ).strip()
    ...
    def calculate_results(self, clients):
        ...
        self.restaurant.reputation = max(
            Decimal("1.0"), min(Decimal("10.0"), self.restaurant.reputation)
        )
