...
def demo_quality_system():
    ...
    for variant in beef_variants:
        ...
        print(
            f"   Certifications: {', '.join(variant.certifications) if variant.certifications else 'Aucune'}"
        )
    ...
    attractiveness_bonus = quality_manager.get_quality_impact_on_attractiveness(
        quality_score
    )
...
def demo_seasonality_system():
    ...
    for test_date, season_name in test_dates:
        ...
        final_price = seasonality_manager.calculate_final_price(
            "tomato", base_price_tomato, test_date
        )
    ...
    if summary["active_events"]:
        ...
        for event in summary["active_events"]:
            print(
                f"     • {event['ingredient_id']}: {event['event']} ({event['price_impact']:+.0f}%)"
            )
...
def demo_stock_management():
    ...
    quantity_obtained, lots_used = stock_manager.consume_ingredient(
        "beef_ground", Decimal("10")
    )
    ...
    if daily_report["lots_near_expiry"]:
        ...
        for lot in daily_report["lots_near_expiry"]:
            print(
                f"     • {lot.ingredient_id} Lot {lot.lot_number}: {lot.days_until_expiry} jours"
            )
...
def demo_integrated_impact():
    ...
    tomato_seasonal_cost = seasonality_manager.calculate_final_price(
        "tomato", tomato_base_cost, summer_date
    )
    ...
    print(
        f"   TOTAL: {((attractiveness_bonus * tomato_demand_impact) - 1) * 100:+.0f}%"
    )
