...
def simulate_game_session():
    ...
    try:
        ...
        for i, location in enumerate(locations[:3], 1):
            ...
            print(f"     💰 {location.price:,.0f}€ + {location.renovation_cost:,.0f}€ rénovation")
            print(f"     🏠 {location.size} couverts - Loyer: {location.rent_monthly:,.0f}€/mois")
            print(f"     📈 Passage: {location.foot_traffic} - Concurrence: {location.competition_nearby}")
            print()
        ...
        print(f"\n📊 SIMULATION DES RÉSULTATS:")
        print("=" * 40)
        print(f"{'Restaurant':<20} {'Demande':<8} {'Servi':<8} {'Util.':<8} {'CA €':<10}")
        print("-" * 60)
        ...
        print("Le jeu est entièrement fonctionnel avec toutes les nouvelles fonctionnalités !")
...
def test_admin_mode():
    ...
    try:
        ...
        print(f"  • Budget: {settings.starting_budget_min:,.0f}€ - {settings.starting_budget_max:,.0f}€")
