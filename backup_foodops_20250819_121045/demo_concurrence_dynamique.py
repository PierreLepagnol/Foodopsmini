...
from src.foodops_pro.domain.competition import (
    CompetitionManager,
    EventType,
    EventImpact,
)
...
def demo_market_events():
    ...
    for event_type, events in events_by_type.items():
        ...
        for event in events:
            impact_icon = {"positif": "📈", "negatif": "📉", "neutre": "⚖️"}[
                event.impact.value
            ]
...
def demo_event_simulation():
    ...
    competition = CompetitionManager(
        random_seed=123
    )  # Seed différent pour plus d'événements
    ...
    print(
        f"   Événements positifs: {len([e for e in all_events if e.impact == EventImpact.POSITIVE])}"
    )
    print(
        f"   Événements négatifs: {len([e for e in all_events if e.impact == EventImpact.NEGATIVE])}"
    )
    print(
        f"   Événements neutres: {len([e for e in all_events if e.impact == EventImpact.NEUTRAL])}"
    )
    ...
    for turn in range(1, 8):
        ...
        if actions:
            ...
            for action in actions:
                ...
                print(
                    f"      Description: {action.parameters.get('description', 'N/A')}"
                )
                ...
                if action.action_type == "price_reduction":
                    ...
                    print(
                        f"      Boost marketing: +{boost:.1f}% pendant {duration} jours"
                    )
                    ...
                    print(
                        f"      Nouveaux plats: {new_options}, attractivité: +{boost:.1f}%"
                    )
...
def demo_integrated_impact():
    ...
    for turn in range(1, 6):
        ...
        print(
            f"   Satisfaction: {restaurant_performance['customer_satisfaction']:.1f}/5"
        )
