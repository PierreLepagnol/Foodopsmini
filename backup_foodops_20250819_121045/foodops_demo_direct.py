    ...
    def simulate_turn(self, restaurant):
        ...
        for segment_name, segment in self.segments.items():
            customers = self._calculate_segment_customers(
                restaurant, segment_name, segment
            )
        ...
        restaurant.reputation = max(
            1.0, min(10.0, restaurant.reputation + reputation_change)
        )
...
class GameInterface:
    ...
    def get_decisions(self):
        ...
        try:
            ...
            # Qualité
            new_quality = input(
                f"⭐ Niveau qualité 1-5 (actuel: {self.restaurant.quality_level}): "
            )
            ...
            # Personnel
            new_staff = input(
                f"👥 Niveau personnel 1-3 (actuel: {self.restaurant.staff_level}): "
            )
            if new_staff.strip():
    ...
    def simulate_and_show_results(self):
        ...
        if results["lost_customers"] > 0:
            print(
                f"😞 Clients perdus: {results['lost_customers']} (capacité insuffisante)"
            )
    ...
    def show_competitors(self):
        ...
        for comp in self.market.competitors:
            print(
                f"• {comp['name']}: {comp['price']:.2f}€, Qualité {comp['quality']}/5"
            )
    ...
    def show_final_results(self):
        ...
        if self.restaurant.history:
            ...
            avg_customers = sum(r["customers"] for r in self.restaurant.history) / len(
                self.restaurant.history
            )
            avg_satisfaction = sum(
                r["satisfaction"] for r in self.restaurant.history
            ) / len(self.restaurant.history)
