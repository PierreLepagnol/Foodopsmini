    ...
    try:
        ...
        while restaurant['tour'] <= 10 and restaurant['budget'] > -5000:
            ...
            if choix == "1":
                ...
                try:
                    # Prix
                    nouveau_prix = input(f"💵 Prix menu (actuel: {restaurant['prix']:.2f}€, Entrée=garder): ")
                    if nouveau_prix.strip():
                        restaurant['prix'] = max(5.0, min(30.0, float(nouveau_prix)))
                    
                    # Qualité
                    nouvelle_qualite = input(f"⭐ Qualité 1-5 (actuel: {restaurant['qualite']}, Entrée=garder): ")
                    if nouvelle_qualite.strip():
                        restaurant['qualite'] = max(1, min(5, int(nouvelle_qualite)))
                    
                    # Personnel
                    nouveau_personnel = input(f"👥 Personnel 1-3 (actuel: {restaurant['personnel']}, Entrée=garder): ")
                ...
                for nom_segment, segment in segments.items():
                    ...
                    clients_segment = int(segment['taille'] * attractivite_globale * bruit)
                ...
                cout_ingredients = clients_servis * 4.50 * modificateur_qualite.get(restaurant['qualite'], 1.0)
                ...
                satisfaction = satisfaction_base + (facteur_prix - 1) * 0.5 + bonus_qualite.get(restaurant['qualite'], 0.0)
                satisfaction = max(1.0, min(5.0, satisfaction))
                
                # Réputation
                changement_reputation = (satisfaction - 3.0) * 0.1
                restaurant['reputation'] = max(1.0, min(10.0, restaurant['reputation'] + changement_reputation))
                ...
                if clients_perdus > 0:
                    print(f"😞 Clients perdus: {clients_perdus} (capacité insuffisante)")
