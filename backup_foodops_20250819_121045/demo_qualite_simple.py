...
def demo_restaurant_quality():
    ...
    print("-" * 100)
    print(f"{'Restaurant':<20} | {'Type':<12} | {'Qualité':<15} | {'Prix':<8} | {'Coût':<8} | {'Attractivité':<12}")
    print("-" * 100)
    ...
    for resto in restaurants:
        ...
        print(f"{resto.name:<20} | {resto.type.value:<12} | {quality_desc:<15} | {ticket:>6.2f}€ | {cost_impact:>6.0%} | S:{students_factor:.2f} F:{families_factor:.2f} G:{foodies_factor:.2f}")
        
        # Détail des ingrédients
        if resto.ingredient_choices:
            ingredients = ", ".join([f"{k.split('_')[-1]}:{v}⭐" for k, v in resto.ingredient_choices.items()])
...
def demo_satisfaction_evolution():
    ...
    print(f"{'Période':<8} | {'Satisfaction':<12} | {'Réputation':<11} | {'Évolution':<10}")
    print("-" * 50)
    ...
    for i, satisfaction in enumerate(satisfactions, 1):
        ...
        print(f"{i:<8} | {satisfaction:<12.1f} | {resto.reputation:<11.1f} | {evolution:+.2f}")
...
def demo_seasonal_impact():
    ...
    for month, month_name in months:
        ...
        print(f"{month_name:<10} | {final_price:>6.2f}€ | {quality_bonus:+d}⭐ | {demand_change:+5.0f}%")
...
def demo_strategic_comparison():
    ...
    print(f"{'Restaurant':<20} | {'Étudiants':<10} | {'Familles':<10} | {'Foodies':<10} | {'Score':<8}")
    print("-" * 70)
    ...
    for resto in restaurants:
        ...
        print(f"{resto.name:<20} | {scores[0]:>8.2f} | {scores[1]:>8.2f} | {scores[2]:>8.2f} | {total_score:>6.2f}")
