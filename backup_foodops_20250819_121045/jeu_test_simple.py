...
def choisir_commerce():
    ...
    for i, commerce in enumerate(commerces, 1):
        ...
        print(f"   💰 {commerce['prix']:,}€ + {commerce['renovation']:,}€ rénovation = {total:,}€")
        print(f"   🏠 {commerce['couverts']} couverts - Loyer: {commerce['loyer']:,}€/mois")
        print(f"   📈 Passage: {commerce['passage']} - Concurrence: {commerce['concurrence']}")
    ...
    while True:
        try:
            ...
            choix = int(input())
            if 1 <= choix <= 3:
                return commerces[choix - 1], budget - (commerces[choix - 1]["prix"] + commerces[choix - 1]["renovation"])
...
def modifier_prix(restaurant):
    ...
    try:
        ...
        if 1 <= choix <= 3:
            ...
            print(f"Nouveau prix pour {plat} (actuel: {ancien_prix:.2f}€): ", end="", flush=True)
            nouveau_prix = float(input())
            
            restaurant['menu'][choix - 1] = (plat, nouveau_prix)
            changement = ((nouveau_prix - ancien_prix) / ancien_prix) * 100
            print(f"✅ Prix modifié: {ancien_prix:.2f}€ → {nouveau_prix:.2f}€ ({changement:+.1f}%)")
        else:
            print("❌ Choix invalide.")
    except ValueError:
        print("❌ Prix invalide.")
