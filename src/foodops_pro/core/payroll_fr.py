...
from ..domain.employee import Employee, EmployeeContract, EmployeePosition
...
class PayrollCalculator:
    ...
    def calculate_payroll(self, employee: Employee, 
                         hours_worked: Decimal = Decimal("151.67"),
                         sunday_hours: Decimal = Decimal("0"),
                         period: str = "") -> PayrollResult:
        """
    ...
    def _calculate_overtime_pay(self, employee: Employee, overtime_hours: Decimal) -> Decimal:
        """
        ...
        overtime_pay = (
            hours_25 * hourly_rate * (self.overtime_rate_25 - 1) +
            hours_50 * hourly_rate * (self.overtime_rate_50 - 1)
        )
        
        return overtime_pay
    
    def _calculate_sunday_premium(self, employee: Employee, sunday_hours: Decimal) -> Decimal:
        """
    ...
    def _calculate_employee_charges(self, employee: Employee, gross_salary: Decimal) -> Decimal:
        """
    ...
    def _calculate_employer_charges(self, employee: Employee, gross_salary: Decimal) -> Decimal:
        """
    ...
    def calculate_team_payroll(self, employees: List[Employee], 
        ...
        for employee in employees:
            hours = hours_per_employee.get(employee.id, Decimal("151.67")) if hours_per_employee else Decimal("151.67")
            sunday_hours = sunday_hours_per_employee.get(employee.id, Decimal("0")) if sunday_hours_per_employee else Decimal("0")
    ...
    def get_payroll_summary(self, payroll_results: List[PayrollResult]) -> Dict[str, Decimal]:
        """
        ...
        total_employer_charges = sum(result.social_charges_employer for result in payroll_results)
        ...
        return {
            ...
            'average_charge_rate': (total_employer_charges / total_gross * 100) if total_gross > 0 else Decimal("0")
        }
