...
class FoodOpsProGame:
    ...
    def _commerce_selection_phase(self) -> None:
        ...
        num_players = self.ui.get_input(
            ...
            int, min_val=1, max_val=max_players,
            default=1
        )
    ...
    def _select_commerce_for_player(self, player_num: int) -> None:
        ...
        budget = self.ui.get_input(
            ...
            default=(self.admin_settings.starting_budget_min + self.admin_settings.starting_budget_max) / 2
        )
        ...
        if not available_locations:
            self.ui.show_error(f"Aucun commerce disponible avec un budget de {budget:.0f}€")
            return
    ...
    def _confirm_commerce_purchase(self, location: CommerceLocation, budget: Decimal) -> bool:
    ...
    def _create_restaurant_from_commerce(self, location: CommerceLocation, 
                                       budget: Decimal, player_num: int) -> Restaurant:
        """Crée un restaurant à partir d'un commerce acheté."""
        # Nom du restaurant
        restaurant_name = self.ui.get_input(
            f"Nom de votre restaurant",
            default=f"Restaurant {player_num}"
        )
    ...
    def _add_base_employees(self, restaurant: Restaurant, restaurant_type: RestaurantType) -> None:
        ...
        employee_configs = base_configs.get(restaurant_type, base_configs[RestaurantType.CLASSIC])
    ...
    def _setup_base_menu(self, restaurant: Restaurant, restaurant_type: RestaurantType) -> None:
        ...
        recipes_for_type = menu_configs.get(restaurant_type, menu_configs[RestaurantType.CLASSIC])
    ...
    def _update_restaurants(self, results: Dict) -> None:
        ...
        for restaurant in self.players + self.ai_competitors:
            if restaurant.id in results:
                ...
                profit = result.revenue - variable_costs - (fixed_costs / 4) - (personnel_costs / 4)
    ...
    def _end_game(self) -> None:
        ...
        if winner in self.players:
            self.ui.show_success(f"🎉 Félicitations ! '{winner.name}' remporte la partie !")
        else:
            self.ui.show_info(f"🤖 L'IA '{winner.name}' remporte cette partie. Réessayez !")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="FoodOps Pro - Simulateur de gestion de restaurant")
    parser.add_argument("--scenario", type=Path, help="Chemin vers le fichier de scénario")
