...
from .persistence import GameState, DecimalEncoder
...
class KPICalculator:
    ...
    @staticmethod
    def calculate_restaurant_kpis(restaurant_data: Dict, 
                                turn_history: List[Dict]) -> Dict[str, Any]:
        """
        ...
        avg_utilization = sum(turn['utilization_rate'] for turn in restaurant_turns) / len(restaurant_turns)
        ...
        return {
            ...
            'utilization_consistency': 1 - utilization_std,  # Plus proche de 1 = plus régulier
            'best_turn': {
                'turn': best_turn['turn'],
    ...
    def calculate_market_kpis(turn_history: List[Dict]) -> Dict[str, Any]:
        ...
        avg_market_utilization = sum(turn['market_utilization'] for turn in market_turns) / len(market_turns)
        avg_demand_satisfaction = sum(turn['demand_satisfaction'] for turn in market_turns) / len(market_turns)
...
class ResultsExporter:
    ...
    def export_to_csv(self, game_state: GameState, output_dir: Path) -> List[Path]:
        ...
        with open(turns_file, 'w', newline='', encoding='utf-8') as file:
            if game_state.turn_history:
                ...
                restaurant_names.update({r['id']: r['name'] for r in game_state.ai_competitors})
                ...
                for turn_data in game_state.turn_history:
                    for restaurant_id, result in turn_data['results'].items():
                        row = {
                            ...
                            'restaurant_name': restaurant_names.get(restaurant_id, 'Unknown'),
                            **result
                        }
                        writer.writerow(row)
        ...
        with open(kpis_file, 'w', newline='', encoding='utf-8') as file:
            fieldnames = [
                'restaurant_id', 'restaurant_name', 'restaurant_type',
                'total_revenue', 'total_customers', 'average_ticket',
                'avg_utilization_rate', 'revenue_growth_percent',
                'utilization_consistency', 'turns_played'
            ]
            ...
            for restaurant in all_restaurants:
                ...
                if kpis:
                    row = {
                        ...
                        **{k: v for k, v in kpis.items() if k not in ['best_turn', 'worst_turn']}
                    }
                    writer.writerow(row)
    ...
    def export_to_json(self, game_state: GameState, output_path: Path) -> None:
        ...
        with open(output_path, 'w', encoding='utf-8') as file:
            json.dump(report, file, cls=DecimalEncoder, 
