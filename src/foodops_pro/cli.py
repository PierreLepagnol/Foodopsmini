...
class FoodOpsGame:
    ...
    def __init__(self, scenario_path: Optional[Path] = None, debug: bool = False) -> None:
        """
        Initialise le jeu.
    ...
    def _create_restaurant(self, name: str, rest_type: RestaurantType, 
                          is_player: bool = True) -> Restaurant:
        """
    ...
    def _setup_base_menu(self, restaurant: Restaurant) -> None:
        ...
        recipes_for_type = base_recipes.get(restaurant.type, base_recipes[RestaurantType.CLASSIC])
    ...
    def _create_ai_competitors(self) -> None:
        ...
        for i, (name, rest_type) in enumerate(ai_configs[:self.scenario.ai_competitors]):
    ...
    def _player_decisions(self) -> None:
        ...
        for i, player in enumerate(self.players):
            ...
            if active_menu:
                adjust_prices = input("Ajuster les prix ? (o/N) : ").lower().startswith('o')
                if adjust_prices:
    ...
    def _adjust_menu_prices(self, restaurant: Restaurant) -> None:
        ...
        for recipe_id, current_price in active_menu.items():
            ...
            while True:
                try:
                    new_price_str = input("Nouveau prix (Entrée pour garder) : ").strip()
    ...
    def _display_turn_results(self, results: Dict) -> None:
        ...
        print("-" * 80)
        print(f"{'Restaurant':<20} {'Demande':<8} {'Servi':<8} {'Capacité':<10} {'Util.':<6} {'CA €':<10}")
        print("-" * 80)
        ...
        print(f"Marché total : {market_analysis['total_served']} clients servis, "
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="FoodOps Pro - Jeu de gestion de restaurant")
