...
class SeasonalModifier:
    ...
    def __post_init__(self):
        ...
        if self.price_multiplier <= 0:
            raise ValueError(f"Le multiplicateur de prix doit être positif: {self.price_multiplier}")
        if not (-2 <= self.quality_bonus <= 2):
            raise ValueError(f"Le bonus qualité doit être entre -2 et +2: {self.quality_bonus}")
        if self.availability_multiplier < 0:
            raise ValueError(f"La disponibilité doit être positive: {self.availability_multiplier}")
...
class SeasonalityManager:
    ...
    def _load_default_seasonality(self):
        ...
        for veg_id, price_mult, quality_bonus in spring_vegetables:
            self.seasonal_modifiers.append(SeasonalModifier(
        ...
        self.event_modifiers.extend([
            EventModifier("salmon_fillet", SpecialEvent.CHRISTMAS, 12, 1, Decimal("1.50"), Decimal("1.8")),
            EventModifier("cheese_goat", SpecialEvent.CHRISTMAS, 12, 1, Decimal("1.30"), Decimal("1.5")),
        ])
        
        # Pâques
        self.event_modifiers.append(
            EventModifier("egg", SpecialEvent.EASTER, 3, 4, Decimal("1.20"), Decimal("1.3"))
        )
        
        # Canicule (impact sur les salades)
        self.event_modifiers.extend([
            EventModifier("lettuce_iceberg", SpecialEvent.HEATWAVE, 7, 8, Decimal("1.10"), Decimal("1.4")),
            EventModifier("tomato", SpecialEvent.HEATWAVE, 7, 8, Decimal("1.05"), Decimal("1.3")),
        ])
        
        # Rentrée (légumes)
        self.event_modifiers.append(
            EventModifier("potato", SpecialEvent.BACK_TO_SCHOOL, 9, 9, Decimal("1.20"), Decimal("1.2"))
        )
    ...
    def get_seasonal_modifier(self, ingredient_id: str, current_date: Optional[date] = None) -> Optional[SeasonalModifier]:
        ...
        for modifier in self.seasonal_modifiers:
            if modifier.ingredient_id == ingredient_id and modifier.season == current_season:
                return modifier
        
        return None
    
    def get_active_event_modifiers(self, ingredient_id: str, current_date: Optional[date] = None) -> List[EventModifier]:
        ...
        for modifier in self.event_modifiers:
            if modifier.ingredient_id == ingredient_id and modifier.is_active(current_month):
                active_modifiers.append(modifier)
        
        return active_modifiers
    
    def calculate_final_price(self, ingredient_id: str, base_price: Decimal, 
    ...
    def get_quality_bonus(self, ingredient_id: str, current_date: Optional[date] = None) -> int:
        """Retourne le bonus de qualité saisonnier."""
        seasonal_mod = self.get_seasonal_modifier(ingredient_id, current_date)
        return seasonal_mod.quality_bonus if seasonal_mod else 0
    
    def get_availability_multiplier(self, ingredient_id: str, current_date: Optional[date] = None) -> Decimal:
        """Retourne le multiplicateur de disponibilité."""
        seasonal_mod = self.get_seasonal_modifier(ingredient_id, current_date)
        return seasonal_mod.availability_multiplier if seasonal_mod else Decimal("1.0")
    
    def get_demand_impact(self, ingredient_id: str, current_date: Optional[date] = None) -> Decimal:
