...
class QualityModifiers:
    ...
    def __post_init__(self):
        ...
        if self.cost_multiplier <= 0:
            raise ValueError(f"Le multiplicateur de coût doit être positif: {self.cost_multiplier}")
        if self.prep_time_multiplier <= 0:
            raise ValueError(f"Le multiplicateur de temps doit être positif: {self.prep_time_multiplier}")
        if self.shelf_life_multiplier <= 0:
            raise ValueError(f"Le multiplicateur de DLC doit être positif: {self.shelf_life_multiplier}")
...
class IngredientQualityManager:
    ...
    def _load_default_variants(self):
        ...
        self.add_variant(IngredientVariant(
    ...
    def get_variants_for_ingredient(self, base_ingredient_id: str) -> List[IngredientVariant]:
    ...
    def get_variants_by_quality(self, quality_level: QualityLevel) -> List[IngredientVariant]:
        """Retourne toutes les variantes d'un niveau de qualité."""
        return [v for v in self.variants.values() 
                if v.quality_level == quality_level]
    
    def calculate_recipe_quality_score(self, ingredient_variants: Dict[str, IngredientVariant]) -> Decimal:
        """
