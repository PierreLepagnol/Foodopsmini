...
class AdvancedStockLot:
    ...
    def __post_init__(self) -> None:
        ...
        if self.expiry_date <= self.purchase_date:
            raise ValueError("La date de péremption doit être postérieure à la date d'achat")
        if not (0 <= self.quality_degradation_rate <= 1):
            raise ValueError(f"Le taux de dégradation doit être entre 0 et 1: {self.quality_degradation_rate}")
    ...
    def shelf_life_percentage(self) -> Decimal:
        ...
        return Decimal(remaining_days) / Decimal(total_days) if total_days > 0 else Decimal("0")
    ...
    def is_promotion_candidate(self, promotion_threshold: Decimal = Decimal("0.3")) -> bool:
    ...
    def get_promotion_price(self, base_price: Decimal, discount_rate: Decimal = Decimal("0.5")) -> Decimal:
...
class AdvancedStockManager:
    ...
    def get_available_quantity(self, ingredient_id: str, exclude_expired: bool = True) -> Decimal:
    ...
    def consume_ingredient(self, ingredient_id: str, quantity_needed: Decimal) -> Tuple[Decimal, List[AdvancedStockLot]]:
        """
        ...
        ingredient_lots = [lot for lot in self.lots 
    ...
    def get_stock_rotation_analysis(self, ingredient_id: str) -> Dict[str, any]:
        ...
        ingredient_lots = [lot for lot in self.lots if lot.ingredient_id == ingredient_id]
        
        if not ingredient_lots:
            return {'average_age': 0, 'oldest_lot_days': 0, 'rotation_rate': 'N/A'}
        
        total_age = sum((date.today() - lot.purchase_date).days for lot in ingredient_lots)
        average_age = total_age / len(ingredient_lots)
        oldest_lot_days = max((date.today() - lot.purchase_date).days for lot in ingredient_lots)
        ...
        return {
            ...
            'near_expiry_count': len([lot for lot in ingredient_lots if lot.is_near_expiry()])
        }
