...
class MarketSegment:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0 <= self.price_sensitivity <= 2):
            raise ValueError(f"La sensibilité prix doit être entre 0 et 2: {self.price_sensitivity}")
        if not (0 <= self.quality_sensitivity <= 2):
            raise ValueError(f"La sensibilité qualité doit être entre 0 et 2: {self.quality_sensitivity}")
...
class Scenario:
    ...
    def __post_init__(self) -> None:
        ...
        if self.base_demand <= 0:
            raise ValueError(f"La demande de base doit être positive: {self.base_demand}")
        if not (0 <= self.demand_noise <= 1):
            raise ValueError(f"Le bruit de demande doit être entre 0 et 1: {self.demand_noise}")
        if not self.segments:
            raise ValueError("Le scénario doit avoir au moins un segment")
        if self.ai_competitors < 0:
            raise ValueError(f"Le nombre de concurrents IA doit être positif: {self.ai_competitors}")
        ...
        if not (0.95 <= total_share <= 1.05):
            raise ValueError(f"Les parts de marché doivent totaliser ~1.0: {total_share}")
        ...
        for contract_type, rate in self.social_charges.items():
            if not (0 <= rate <= 1):
                raise ValueError(f"Les charges sociales doivent être entre 0 et 1: {rate}")
