...
class MarketingCampaign:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0 <= self.expected_conversion <= 1):
            raise ValueError(f"Le taux de conversion doit être entre 0 et 1: {self.expected_conversion}")
    ...
    def get_daily_impact(self, current_date: date) -> Dict[str, Decimal]:
        ...
        if not self.is_active(current_date):
            return {"reach": Decimal("0"), "conversion": Decimal("0"), "cost": Decimal("0")}
...
class MarketingManager:
    ...
    def create_campaign(self, name: str, campaign_type: CampaignType, 
                       budget: Decimal, duration_days: int,
                       target_segments: List[str] = None) -> MarketingCampaign:
        """
        ...
        if not (template["min_duration"] <= duration_days <= template["max_duration"]):
            raise ValueError(f"Durée invalide pour {campaign_type.value}: "
    ...
    def calculate_daily_marketing_impact(self, current_date: date) -> Dict[str, any]:
        ...
        for campaign in active_campaigns:
            ...
            for segment in campaign.target_segments:
                ...
                segment_impact[segment] += impact["conversion"] / len(campaign.target_segments)
    ...
    def add_customer_review(self, customer_id: str, restaurant_id: str,
                           rating: Decimal, comment: str, platform: str = "google") -> None:
    ...
    def _update_reputation_score(self) -> None:
        ...
        for review in self.reviews[-50:]:  # 50 derniers avis
            ...
            time_weight = max(Decimal("0.1"), Decimal("1.0") - Decimal(days_old) / Decimal("365"))
    ...
    def get_reputation_summary(self) -> Dict[str, any]:
        ...
        older_reviews = [r for r in self.reviews if 30 < (date.today() - r.date).days <= 60]
        
        recent_avg = sum(r.rating for r in recent_reviews) / len(recent_reviews) if recent_reviews else Decimal("3.0")
        older_avg = sum(r.rating for r in older_reviews) / len(older_reviews) if older_reviews else Decimal("3.0")
    ...
    def get_marketing_roi(self, period_days: int = 30) -> Dict[str, any]:
        ...
        total_spend = sum(
            c.budget for c in self.campaigns 
        ...
        return {
            ...
            "roi_ratio": estimated_revenue / total_spend if total_spend > 0 else Decimal("0"),
            "campaigns_completed": len([c for c in self.campaigns if c.status == CampaignStatus.COMPLETED])
        }
