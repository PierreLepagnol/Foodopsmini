...
class RecipeItem:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0 < self.rendement_prepa <= 1):
            raise ValueError(f"Le rendement prépa doit être entre 0 et 1: {self.rendement_prepa}")
        if not (0 < self.rendement_cuisson <= 1):
            raise ValueError(f"Le rendement cuisson doit être entre 0 et 1: {self.rendement_cuisson}")
...
class Recipe:
    ...
    def __post_init__(self) -> None:
        ...
        if self.temps_prepa_min < 0:
            raise ValueError(f"Le temps de prépa doit être positif: {self.temps_prepa_min}")
        if self.temps_service_min < 0:
            raise ValueError(f"Le temps de service doit être positif: {self.temps_service_min}")
        if self.portions <= 0:
            raise ValueError(f"Le nombre de portions doit être positif: {self.portions}")
    ...
    def get_ingredient_quantity(self, ingredient_id: str) -> Decimal:
        ...
        raise ValueError(f"Ingrédient {ingredient_id} non trouvé dans la recette {self.id}")
