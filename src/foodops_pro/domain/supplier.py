...
class Supplier:
    ...
    def __post_init__(self) -> None:
        ...
        if self.lead_time_days < 0:
            raise ValueError(f"Le délai de livraison doit être positif: {self.lead_time_days}")
        if self.min_order_value < 0:
            raise ValueError(f"Le MOQ doit être positif: {self.min_order_value}")
        if self.shipping_cost < 0:
            raise ValueError(f"Les frais de port doivent être positifs: {self.shipping_cost}")
        if self.payment_terms_days < 0:
            raise ValueError(f"Le délai de paiement doit être positif: {self.payment_terms_days}")
        
        if self.discount_threshold is not None:
            if self.discount_threshold <= 0:
                raise ValueError(f"Le seuil de remise doit être positif: {self.discount_threshold}")
            if self.discount_rate is None or not (0 <= self.discount_rate <= 1):
                raise ValueError(f"Le taux de remise doit être entre 0 et 1: {self.discount_rate}")
    ...
    def calculate_total_cost(self, order_value_ht: Decimal) -> Decimal:
        ...
        if order_value_ht < self.min_order_value:
            raise ValueError(f"Commande {order_value_ht} inférieure au MOQ {self.min_order_value}")
        
        # Application de la remise si applicable
        discounted_value = order_value_ht
        if (self.discount_threshold is not None and 
            self.discount_rate is not None and 
            order_value_ht >= self.discount_threshold):
