...
class FinanceManager:
    ...
    def create_transaction(self, description: str, transaction_type: TransactionType,
    ...
    def record_sale(self, amount: Decimal, payment_method: str = "cash") -> Transaction:
        ...
        credit_account = "530" if payment_method == "cash" else "411"  # Caisse ou Clients
    ...
    def record_purchase(self, amount: Decimal, supplier: str, category: str = "ingredients") -> Transaction:
        """Enregistre un achat."""
        expense_account = "601" if category == "ingredients" else "602"  # Matières premières ou emballages
    ...
    def record_marketing_expense(self, amount: Decimal, campaign_name: str) -> Transaction:
    ...
    def get_income_statement(self, start_date: date = None, end_date: date = None) -> Dict[str, any]:
        ...
        return {
            ...
            "margins": {
                "gross_margin": revenues - expense_accounts.get("Achats matières premières", Decimal("0")),
                "operating_margin": net_profit,
                "net_margin_rate": net_profit / revenues if revenues > 0 else Decimal("0")
            }
        }
    ...
    def update_recipe_profitability(self, recipe_id: str, recipe_name: str,
                                  selling_price: Decimal, ingredient_cost: Decimal,
                                  labor_cost: Decimal, overhead_cost: Decimal) -> None:
    ...
    def get_cash_flow_forecast(self, days: int = 30) -> Dict[str, any]:
        ...
        recent_transactions = [
            t for t in self.transactions[-100:]  # 100 dernières transactions
        ]
        
        daily_inflows = sum(
            t.amount for t in recent_transactions
            if t.type == TransactionType.SALE
        ) / len(recent_transactions) if recent_transactions else Decimal("0")
    ...
    def get_financial_ratios(self) -> Dict[str, Decimal]:
        ...
        # Ratios de liquidité
        ratios["current_ratio"] = assets / liabilities if liabilities > 0 else Decimal("0")
        
        # Ratios de rentabilité
        ratios["net_margin"] = net_profit / revenues if revenues > 0 else Decimal("0")
        ratios["roe"] = net_profit / equity if equity > 0 else Decimal("0")  # Return on Equity
        ratios["roa"] = net_profit / assets if assets > 0 else Decimal("0")  # Return on Assets
