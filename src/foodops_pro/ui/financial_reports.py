...
class FinancialReports:
    ...
    def show_profit_loss_statement(self, restaurant: Restaurant, ledger: Ledger, 
                                 period: str = "Mois en cours") -> None:
    ...
    def _calculate_detailed_metrics(self, restaurant: Restaurant, 
                                  pnl_data: Dict, balance_data: Dict) -> Dict[str, Decimal]:
        ...
        metrics['achats_mp'] = total_expenses * Decimal("0.30")  # 30% pour matières premières
        ...
        metrics['total_charges'] = (
            metrics['achats_mp'] + metrics['loyer'] + metrics['energie'] +
            metrics['assurances'] + metrics['marketing'] + metrics['autres_externes'] +
            metrics['salaires_bruts'] + metrics['charges_sociales'] +
            metrics['amortissements'] + metrics['autres_charges']
        )
    ...
    def _show_business_kpis(self, restaurant: Restaurant, metrics: Dict[str, Decimal]) -> None:
        ...
        if ca_ht > 0:
            ...
            personnel_cost_pct = ((metrics['salaires_bruts'] + metrics['charges_sociales']) / ca_ht * 100)
            marge_brute_pct = ((ca_ht - metrics['achats_mp']) / ca_ht * 100)
            marge_nette_pct = (metrics['resultat_net'] / ca_ht * 100)
        else:
            food_cost_pct = personnel_cost_pct = marge_brute_pct = marge_nette_pct = Decimal("0")
        
        # Ticket moyen estimé
        if hasattr(restaurant, '_last_customers_served') and restaurant._last_customers_served > 0:
    ...
    def _show_financial_analysis(self, metrics: Dict[str, Decimal]) -> None:
        ...
        if ca_ht > 0:
            personnel_pct = (metrics['salaires_bruts'] + metrics['charges_sociales']) / ca_ht * 100
            if personnel_pct > 40:
                analysis.append(f"❌ Charges personnel trop élevées ({personnel_pct:.1f}%)")
                ...
                analysis.append(f"✅ Charges personnel maîtrisées ({personnel_pct:.1f}%)")
    ...
    def show_cash_flow_statement(self, restaurant: Restaurant, ledger: Ledger) -> None:
        ...
        if restaurant.cash < restaurant.rent_monthly:
            ...
            style = "error"
        elif restaurant.cash < (restaurant.rent_monthly + restaurant.fixed_costs_monthly) * 2:
