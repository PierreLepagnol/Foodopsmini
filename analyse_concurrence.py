...
SEGMENTS = [
    {"name": "Étudiants", "share": 0.35, "budget": 11.0, "type_weight": {"fast": 1.20, "classic": 0.70}},
    {"name": "Familles", "share": 0.40, "budget": 17.0, "type_weight": {"fast": 0.90, "classic": 1.00}},
    {"name": "Foodies", "share": 0.25, "budget": 25.0, "type_weight": {"fast": 0.60, "classic": 1.30}},
]
...
def allocate_demand_detailed(restos: List[Restaurant], total_demand: int, rng: random.Random) -> Dict:
    ...
    for r in restos:
        ...
        print(f"  {r.name}: {allocated} demandé, {capacity} capacité → {served} servi {status}")
...
def test_concurrence_scenarios():
    ...
    for scenario in scenarios:
        ...
        for resto in scenario['restos']:
            ...
            total_ca += ca
            market_share = resto_result['served'] / result['total_served'] * 100 if result['total_served'] > 0 else 0
